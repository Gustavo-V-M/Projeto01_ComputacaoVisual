...
void Renderer::draw_button(SDL_Renderer *renderer, const SDL_Rect &button_rectangle, bool hovered, bool pressed,
    ...
    if (font) {
        ...
        SDL_FRect text_rect{
            float_rectangle.x + (float_rectangle.w - text_width) / 2.0f,
            float_rectangle.y + (float_rectangle.h - text_height) / 2.0f,
            (float) text_width,
            (float) text_height
        };
...
void Renderer::render_texture_fit(SDL_Renderer *renderer, SDL_Texture *texture, float area_x, float area_y,
...
void Renderer::render_secondary(SDL_Renderer *renderer, SDL_Texture *texture, SDL_Window *window, TTF_Font *font,
                                char *text) {
    if (!renderer || !window)
        return;
    ...
    draw_button(renderer, button, hovered, pressed, font);
    SDL_RenderPresent(renderer);
}
